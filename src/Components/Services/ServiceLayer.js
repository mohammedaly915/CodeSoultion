...
const ServiceLayer = ({ service, index, totalServices }) => {
  ...
  return (
    ...
    >
      <motion.div 
        className="relative w-[85%] max-w-4xl bg-gradient-to-br from-primeColor/95 to-primeColor/90 backdrop-blur-3xl rounded-2xl p-px shadow-2xl overflow-hidden"
      ...
      >
        ...
        <div className="relative rounded-[calc(2.5rem-2px)] bg-gradient-to-br from-primeColor to-primeColor/90 p-12 overflow-hidden">
          {/* Header */}
          <motion.div 
            className="absolute top-0 left-0 right-0 h-32 bg-gradient-to-br from-secondColor/30 to-teal-400/20 backdrop-blur-xl rounded-t-2xl flex items-center pl-12"
          ...
          <div className="mt-32">
            ...
            <ul className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {service.specs.map((spec, i) => (
                <motion.li
                  key={i}
                  className="group relative p-6 bg-teal-400/5 transition-all duration-300 hover:bg-teal-400/10 rounded-xl flex items-start"
