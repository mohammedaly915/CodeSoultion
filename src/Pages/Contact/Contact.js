  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 to-primeColor py-24 px-6 flex items-center justify-center">
      <motion.div
        ...
        className="w-full max-w-4xl bg-white/10 backdrop-blur-xl rounded-3xl p-10 shadow-2xl grid md:grid-cols-2 gap-12"
      >
        {/* Contact Form */}
        <div>
          <h1 className="text-5xl font-extrabold mb-8 bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 to-blue-500">
            Get in Touch
          </h1>
          <motion.form
          ...
          >
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="w-full py-4 px-6 bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-semibold text-lg rounded-xl shadow-lg hover:shadow-xl hover:from-cyan-400 hover:to-blue-500 transition-all"
              type="submit"
            >
              Send Message
            </motion.button>
...
const AnimatedInput = ({ label, type = 'text', value, onChange, required }) => {
  ...
  return (
    ...
    >
      <motion.label
        className={`absolute left-4 top-3 transition-all text-gray-300 ${
          isFocused || value ? 'text-sm text-cyan-400 -top-2' : 'text-base top-4'
        }`}
      >
        {label}
      </motion.label>
      {type === 'textarea' ? (
        <motion.textarea
          className="w-full bg-white/10 rounded-xl p-4 pt-6 text-white focus:outline-none focus:ring-2 focus:ring-cyan-400 resize-none"
          value={value}
          onChange={onChange}
        ...
        <motion.input
          className="w-full bg-white/10 rounded-xl p-4 pt-6 text-white focus:outline-none focus:ring-2 focus:ring-cyan-400"
          type={type}
          value={value}
          onChange={onChange}
          onFocus={() => setIsFocused(true)}
